...
Output Format:
For each test case/query, the only line of output prints the order in which the queue elements are dequeued, all of them
...
void reverseQueue(queue<int> &input){
	if(input.size() == 0 || input.size() == 1){	//Using Recursion.
		return;
	}
